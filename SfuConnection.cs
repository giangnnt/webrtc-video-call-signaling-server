...
public class SfuConnection
{
    ...
    public WebSocketState ConnectionState => _ws.State;
    ...
    private async Task ForwardToClientAsync(string clientMethod, string sdp)
    {
        try
        {
        ...
        {
            _logger.LogError(ex, $"Failed to forward to client: {clientMethod}");
            throw;
        }
    }
    ...
    public async Task Trickle(string candidateJson, int target)
    {
        _logger.LogDebug($"Sending trickle candidate for target {target}: {candidateJson}");
        ...
        await SendJsonRpcAsync(rpc.ToJson());
        _logger.LogDebug($"Trickle candidate sent for target {target}");
    }
